...
namespace EditorGrafos
{
    ...
    public class Grafo
    {
        ...
        private List<Nodo> Lk, Lm;


        public Grafo(Form1 f)
        {
        ...
        public void copiaGrafo(Grafo g)
        {
        ...
        public void fucionaNodos(Nodo n)
        {
            Nodo nAux = new Nodo();
            //CNodo aux = new CNodo();
        ...
        public bool IsomorfismoManual(Grafo g1, Grafo g2)
        {
            g1.generaMatriz();
            g2.generaMatriz();
        ...
        public Arista eliminaAristaMR(int r, int c)
        {
            Nodo nodoR = this.ListaNodos.ElementAt<Nodo>(r);
            Nodo nodoC = this.ListaNodos.ElementAt<Nodo>(c);
            int i, j;
            Arista auxAri=new Arista();
            auxAri = null;
            int elimina = -1;
            
            foreach (Arista ari in ListaAristas)
            {
            ...
            if (elimina != -1)
            {
                auxAri = this.ListaAristas.ElementAt<Arista>(elimina);
        ...
        private void agregaAristaMR(Arista arista)
        {
            this.ListaAristas.Add(arista);
            this.generaMatriz();
        }

        public List<int>adyacentes(Nodo n)
        {
            ...
            {
                nAux = ListaNodos.ElementAt<Nodo>(i);
        ...
        public int Aristas(Nodo n)
        {
            ...
            for (int i = 0; i < this.Count; i++)
            {
                nAux = this.ListaNodos.ElementAt<Nodo>(i);
                if(matriz[nod,i]==1 && nAux.visible==true)
                {
                    a++;
                }
        ...
        public bool esPuente(Nodo u, Nodo v)
        {
            bool puente = false;
            int cont, cont2;
            int r, c;
        ...
        public int conteoProfundidad(Nodo v)
        {
            int cont;
            int i;
            ...
            foreach (var rel in rela)
            {
                if(matriz[i,rel]==1)
                {
                    if(ListaNodos.ElementAt<Nodo>(rel).revisado==false)
                    {
                        cont = cont + conteoProfundidad(ListaNodos.ElementAt<Nodo>(rel));
                    }
                }
            }
        ...
        {
            ListaAristas.Add(new Arista(a, b));
        }

        public void addArista(Nodo a, Nodo b, int p)
        {
            ListaAristas.Add(new Arista(a, b, p));
        }

        public void addArista(Nodo a, Nodo b, int p, bool d)
        {
            ListaAristas.Add(new Arista(a, b, p, d));
        }
        ...
        public Nodo buscaNodoPointUp(Point p)
        {
            double dist;
            Nodo n = new Nodo();
            Nodo nod = new Nodo();
            ...
            {
                nod = ListaNodos.ElementAt<Nodo>(i);
                if (nod.visible == true)
                {
        ...
        public Nodo buscaNodoPoint(Point p)
        {
            double dist;
            Nodo n = new Nodo();
            n = null;

            foreach (Nodo nod in this.ListaNodos)
            {
                if (nod.visible == true)
                {
                    dist = Math.Sqrt(Math.Pow(p.X - nod.GSCentro.X, 2) + Math.Pow(p.Y - nod.GSCentro.Y, 2));
        ...
        public void eliminaNodo(Nodo nodo, bool band)
        {
            int i=0;
        ...
        public void eliminaNodoLogic(Nodo nodo, bool band)
        {
            int i = 0;
        ...
        public void pintaCamino(List<int> camino, int ult)
        {
            ...
            for (int i = 0; i <= ult; i++)
            {
                this.ListaNodos.ElementAt<Nodo>(camino[i]).GSColor = Color.Red;
            }
            int del = 0;
            while (del < 100000000)
                del++;
            for (int i = 1; i <= ult; i++)
            {
                ari=new Arista(this.ListaNodos.ElementAt<Nodo>(camino[i - 1]), this.ListaNodos.ElementAt<Nodo>(camino[i]));
        ...
        public void pintaArbol(List<List<int>> arbol)
        {
            for (int i = 0; i < arbol.Count; i++)
            {
                ...
                {
                    this.ListaNodos.ElementAt<Nodo>(arbol[i][j]).GSColor = Color.Red;
                }
            }
        }
        ...
        public void cambiaID(Button b)
        {
            char c = 'A';
            int i = 0;
            if (this.ListaNodos.Count != 0 && this.ListaNodos.ElementAt<Nodo>(0).GSNombre == c.ToString())
            {
                b.Text = "0";
                foreach (Nodo nod in this.ListaNodos)
                {
        ...
        public bool SeRelacionan(Nodo n1, Nodo n2)
        {
